    ...
    const dailyTaskList = (
        <div className="flex flex-col gap-4 rounded-lg px-3 py-4 shadow-[0_-3px_30px_0px_rgba(96,214,217,0.25)] min-w-[38%] h-fit">
            ...
            <ul className="flex flex-col gap-3">
                ...
                (
                    <>
                        <li key={id} className="flex justify-between text-xs font-medium">
                            <p>{task}</p>
                            <p>{points} pts</p>
                        </li>
                        <div key={id + dailyTasks.length} className="w-full border-t-[1.5px] border-[#60D6D9]"></div>
                    </>)
                )}
            </ul>
        </div>
        ...
        <div className="flex flex-col w-[35%] gap-4 rounded-lg px-3 py-4 shadow-[0_-3px_30px_0px_rgba(96,214,217,0.25)] h-fit">
            ...
            <ul className="flex flex-col gap-2">
                {eventsList.map(({id, name, description, date}) => (
                    <li className="flex justify-between items-center shadow-md rounded-lg pl-3 pr-1 py-2">
                        <div classname="">
