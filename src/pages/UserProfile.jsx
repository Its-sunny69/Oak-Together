...
const recentActivitiesList = Array(5).fill({
  activity: 'Enrolled in event "Event Name"',
  date: "March 01, 2025",
});
const certificatesList = Array(5).fill({
  title: 'Enrolled in event "Event Name"',
  date: "March 01, 2025",
});
const gcpList = Array(5).fill({
  activity: 'Sponsored an event "Event Name"',
  pointsEarned: "20",
});
...
const badgeDetailsByRarity = {
  "": {
    ...
    shadowColor: "rgba(42,42,42,0.25)",
    bgImage: "",
  },
  COMMON: {
    nameTextColor: "#3BA5DA",
    shadowColor: "rgba(96,214,217,0.25)",
    bgImage: CommonBadgeBG,
  },
  RARE: {
    nameTextColor: "#AAF19C",
    shadowColor: "rgba(30,74,147,0.25)",
    bgImage: RareBadgeBG,
  },
  EPIC: {
    nameTextColor: "#3BA5DA",
    shadowColor: "rgba(130,30,202,0.25)",
    bgImage: EpicBadgeBG,
  },
  LEGENDARY: {
    nameTextColor: ["#8E47FF", "#287BE3"],
    shadowColor: "rgba(255,99,252,0.25)",
    bgImage: LegendaryBadgeBG,
  },
};

// Overview:

function BadgeAndStats({ activeView, selectedBadge, trophyCount, isSponsor }) {
  ...
  const statsList = [
    isSponsor
      ? {
          ...
          tooltipContent:
            "The number of events you have sponsored or supported, helping to fund and facilitate eco-friendly activities.",
        }
      : {
          ...
          tooltipContent:
            "The total number of events you have actively taken part in, contributing to environmental initiatives and community efforts.",
        },
    {
      ...
      tooltipContent:
        "The count of locations you have identified for environmental activities, such as tree plantations or clean-up drives.",
    },
    {
      ...
      tooltipContent:
        "The total number of locations where you have watered trees or plants, contributing to their growth and sustainability.",
    },
    {
      ...
      tooltipContent:
        "An estimate of the amount of CO₂ emissions you have helped offset or reduce through your eco-friendly activities.",
    },
  ];
  ...
  const {
    ...
    bgImage: badgeBG,
  } = badgeDetailsByRarity[selectedBadgeObj.rarity]; // change later

  const darkShadowColor =
    shadowColor.slice(0, shadowColor.lastIndexOf(",")) + ",1)";
  const textStyleObj = {
      ...
      >
        ...
        >
          <img
            src={Trophy2Png}
            alt=""
            className="absolute left-0 w-full h-[60vh] top-1"
          />
          ...
          <ul className="flex flex-col justify-between gap-2 pt-12 items-start">
            {statsList.map(
              ({ statName, statValue, tooltipId, tooltipContent }) => {
                return (
                  <li key={statName} className="relative flex flex-col w-full">
...
function DropdownListComponent({ listName, listContent }) {
  ...
  const listHeaderStyle =
    "flex justify-start items-center border-b-[1.5px] border-opacity-25 border-[#60D6D9]";
  ...
  const activitiesList = (
    <>
      ...
      {listContent.map(({ activity, date }, index) => (
        <li key={index} className="px-4 text-black">
          <div
            className={
              listElementStyle +
              (index < listContent.length - 1 &&
                "border-b-[1.5px] border-opacity-25 border-[#60D6D9]")
            }
          >
            <div>{activity}</div>
      ...
      {listContent.map(({ title, date }, index) => (
        <li key={index} className="px-4 text-black">
          ...
          >
            ...
            <div
              className={
                "flex justify-between " + (guestView ? "w-[15%]" : "w-[30%]")
              }
            >
              {date}
              {!guestView && (
  ...
  const profileInfo = (
    <div
      className={`flex flex-col ${
        guestView ? "gap-1" : "justify-evently gap-3"
      } px-4 py-3 w-[60%] rounded-lg shadow-[rgba(96,214,217,0.2)_-1px_3px_6px_1px] overflow-auto`}
    >
...
function BadgeCard({
  ...
  return (
    <div
      className={`border-8 rounded-lg animate-fade-up transitonal all ${
        isLocked ? "grayscale" : ""
      }`}
      style={{
        borderColor:
          isSelected && !isLocked && !isTopBadge && !guestView
            ? borderColor
            : "white",
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-2">
          <img className="w-[80%] -mt-4" src={badgeImgUrl} alt="Badge Image" />
...
function BadgeDisplay({ selectedBadge, userId }) {
  // might require pagination in future

  const handleBadgeClick = (badgeId) => {
    dispatch(setPrimaryBadge({ userId, badgeId: badgeId }));
  };

  const { allBadgesByFilter: allBadges, badgesByFilter } = useSelector(
    (state) => state.badge
  );
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getAllBadgesByFilter(33)).then(() =>
      dispatch(getBadgesByFilter(`badgeOwner=${userId}&&pageSize=33`))
    );
  }, []);

  const unlockedBadges = new Set([
    ...badgesByFilter.map((badgeObj) => badgeObj.id),
  ]);
  ...
  return (
    <div className="flex flex-col items-center gap-6 transition-all animate-fade-up">
      {!guestView && selectedBadge && (
        <>
          <div className="w-full text-center text-lg font-semibold -mb-3">
            SELECTED BADGE:
          </div>
          <BadgeCard
      ...
      <div className="w-full text-xl text-center font-semibold -mb-8">
        Select a badge to showcase on your profile:
      </div>
      <div className="flex gap-4 p-6">
        {Object.keys(filteredBadgeLists).map((rarity) => (
          <div key={rarity} className="flex flex-col gap-4">
            {filteredBadgeLists[rarity].map(
              ({ rarity, id, name, description, imageUrl }) => (
                <BadgeCard
                  key={id}
                  id={id}
                  rarity={rarity}
...
function FilterButton({ text, handleClick, isActive }) {
  return (
    <button
      className={
        "w-[16vw] py-2 text-sm transition-all shadow-[rgba(96,214,217,0.2)_0px_3px_6px_3px] rounded-lg hover:bg-[#60D6D9] " +
        (isActive && "bg-[#60D6D9]")
      }
      onClick={handleClick}
    >
  ...
  return (
    <div className="flex w-full pr-2 justify-between">
      ...
      <div className="flex items-end gap-1">
        <img src={ConfettiPng} className="h-5 w-5" />
        <span className="text-gray-600">
          {activeListCategory} {listType}s:
        </span>
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      <FilterButtonRow
        ...
        listCount={listCount}
      />
      <div className="flex flex-col gap-2 transition-all animate-fade-up">
        {children}
      </div>
    </div>
  );
...
function ListItem({
  ...
  const dateBox = (
    <div className="flex flex-col w-[8vw] items-center px-3 pt-3 pb-4 rounded-lg shadow-[rgba(96,214,217,0.2)_2px_0px_4px_0px]">
      <div className="text-[#3BA5DA]">
        {dateArr[0] + ", " + months[+dateArr[1] - 1].toUpperCase()}
      </div>
      <div className="text-5xl font-[500]" style={{ fontFamily: "Bebas Neue" }}>
        {dateArr[2]}
      </div>
    </div>
  );
      ...
      <div className={`lex items-center gap-2 ${isLocation ? "" : "w-[30vw]"}`}>
        {isLocation ? (
          <div>
            <FontAwesomeIcon
              icon={faLayerGroup}
              className="text-[#3BA5DA] -ml-1.5 mr-2"
            />
            {space}
          </div>
        ) : (
  ...
  const detailsNearButton = isLocation ? (
  ...
  const viewButton = (
    <button
      className={`rounded-lg ${gradientBgStyle} px-[1px] py-[6.5px] hover:to-[#60D6D9]`}
    >
      <span className="rounded-[6.5px] px-6 py-2 font-medium text-transparent bg-white hover:text-[#60D6D9] active:text-white active:bg-transparent">
        <span className={`${gradientBgStyle} bg-clip-text`}>View</span>
      </span>
    </button>
  );
...
// Events:
function EventDisplay({ gradientBgStyle, userId, isSponsor }) {
  ...
  const [activeEventListCategory, setActiveEventListCategory] = useState(
    isSponsor ? "Sponsored" : "Participated"
  );
        ...
        <div className="absolute flex items-center gap-2 w-[20%] left-1">
          ...
          >
            {eventPerPage.map((option, index) => (
              <option key={index} value={option} defaultValue={option == 10}>
                {option}
              </option>
            ))}
          </select>
...
// Locations:
function LocationDisplay({ gradientBgStyle, userId }) {
  const filterObjByCategory = {
    Marked: {
      markedBy: userId,
    },
  ...
  const [activeLocationListCategory, setActiveLocationListCategory] =
    useState("Marked");
  ...
  return (
    ...
    >
      ...
      {locationList.map(
        (
          { markedOn, waterAvailability, space, position, name, type },
          index
        ) => (
          <ListItem
            key={index}
    ...
    try {
      ...
      if (newDetails) {
        const detailsResponse = await dispatch(
          updateUserById({ userId, updateData: detailsObj })
        ).unwrap();
...
function DeleteFormModal({ userName, setIsModalVisible, userId }) {
  const dispatch = useDispatch();
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    dispatch(deactivateUser({ userId, email: email, password: password }))
      .unwrap()
      .then(() => {
        toast.success(
          `${userName ?? "User"}'s account has been scheduled for deletion.`
        );
...
function UserProfile() {
  ...
  const nameIconDiv = (
    <div className="flex gap-6">
      <div
        style={{
          backgroundImage: `url(${
            userData?.profilePicture?.url ?? ProfileImg2
          })`,
          backgroundPosition: "center",
          backgroundSize: "cover",
          backgroundRepeat: "no-repeat",
  ...
  const navDiv = (
    <ul className="flex gap-8 font-medium">
      {optionsList.map((option) => (
        <li
          className={
            "cursor-pointer " + (option == activeView ? "text-[#60D6D9]" : "")
          }
  ...
  const viewMap = {
    ...
    Events: (
      <EventDisplay gradientBgStyle={gradientBgStyle} userId={userData?.id}  isSponsor={isSponsor} />
    ),
    Locations: (
      <LocationDisplay
        gradientBgStyle={gradientBgStyle}
    ...
    "Edit Form": (
      <EditForm userData={userData} setIsModalVisible={setIsModalVisible} />
    ),
  };
  ...
  const deleteAccountBtn = (
    <button
      className={
        "mt-2 py-2 px-10 w-fit rounded-lg text-white font-medium bg-gradient-135 from-[#FF0000] to-[#654398] hover:from-[#654398] hover:to-[#FF0000]"
      }
  ...
  return (
    <div className="flex">
      ...
      <div className="relative flex flex-col gap-6 w-full px-4 pt-4 pb-8 items-center">
        ...
        {viewMap[activeView]}
        <div className="w-full">{deleteAccountBtn}</div>
        ...
        {isDeleteFormModalVisible && (
          <div className=" absolute inset-0 backdrop-blur-[2px] bg-gray-600/50 z-10 flex justify-center items-center">
            <DeleteFormModal
              ...
              setIsModalVisible={setIsDeleteFormModalVisible}
            />
          </div>
        )}
      </div>
